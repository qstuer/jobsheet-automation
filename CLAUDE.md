...
2026-09-24 隔離測試分支另有「產品／醫院單格」方案：先前用9張實單、兩次完整
獨立試驗，有限詞彙提示在這兩欄各得到9/9；這**不是**20份全流程通過。
測試分支的20份只讀回測可設定 `JOBSHEET_CONTEXT_FIELD_OCR=1`，令產品與醫院
各以5x單格獨立讀兩次；大欄位卡的讀數仍留私人診斷，但不投這兩欄的票。
兩次不一致便停止配對，不碰OneDrive。正式Stage B未設定此開關，維持原行為。
提示只在執行時從私有設備索引投影已確認產品寫法及醫院短碼供辨字；
客戶詞彙不硬寫進新提示程式。電話、聯絡人、Serial、asset、完整地點及
Asana候選都不進這份提示，模型亦不可補造完整名稱或序號。
2026-09-25 隔離分支實測：修正完整醫院名被簡寫提示干擾後，9份的產品／醫院
各為9/9；20份只讀回測嚴格正確11/20、圖片服務錯誤0、OneDrive寫入0。
已確認且頁數完整的16份中，11份正確、4份未選出工作、1份選錯同設備工作。
後者禁止此改版進正式Stage B；本機已加日期分數防錯閘，尚未推送及實單重測。
詳細匿名結果、測試索引範圍及提交限制見分步進度文件。

2026-09-25 後續本機修正（仍未部署）：醫院全名與確認別名改由私有索引的
`location_directory` 提供，程式不再硬寫院名；只有短碼的 Asana 資料只顯示短碼，
不能憑空補正式全名。最終 Asana 工作日期改為先看正式到期／完成日，兩者皆無
才看描述文字，避免舊保養日期影響當次工作。多次歷史工作只靠較遠日期分數
分開時會保留待核對；即使索引按日期先排除了另一筆近三個月的同類工作，
也不能讓唯一剩餘候選繞過防錯閘。這些修改通過本機合成測試，**未做 B08／20份雲端實單
重測，不能聲稱已修好錯月份配對**；正式 main 和 OneDrive 仍未改動。
