...
電話是強輔助、聯絡人是低權重輔助、Asset 有填才加強。ACTION DATE 不參與選設備；
鎖定設備後才用它在該 Serial 的歷史工作中找前後不超過 31 日的工作，再核對 PM/CM、
...
6. 完成/未完成都可以是正確工作。`modified_at` 不代表服務日期；ACTION DATE 不可用來選設備，只在設備確定後比較該 Serial 下面的 Asana 描述、start/due 日期，前後超過 31 日的工作排除。
